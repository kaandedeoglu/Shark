        ...
        switch self {
            case .image(let name, let value):
                switch framework {
                    ...
                    case .appkit:
                        return #"\#(String.indent(indentLevel))public static var \#(name): NSImage { return NSImage(named:"\#(value)")! }"#
                    case .swiftui:
                        return #"\#(String.indent(indentLevel))public static var \#(name): Image { return Image("\#(value)", bundle: bundle) }"#
                }
...
enum ImageEnumBuilder {
    ...
    static func imageEnumString(forFilesAtPaths paths: [String], topLevelName: String, options: Options) throws -> String? {
        ...
        var result = try enumString(for: rootNode, framework: options.framework)
        result.removeLast()
        return result
    }
    
    private static func enumString(for node: Node<ImageValue>, indentLevel: Int = 0, framework: Framework) throws -> String {
        switch node.value {
            case .namespace:
                let childrenString = try node.children.map { try enumString(for: $0, indentLevel: indentLevel + 1, framework: framework) }
